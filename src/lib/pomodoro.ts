...
export const FOCUS_SESSION_IDLE_TTL_MS = 3 * 60 * 60 * 1000;

async function expireIdleFocusSessions() {
  const idleSessions = {
    isActive: true,
    updatedAt: { lt: new Date(Date.now() - FOCUS_SESSION_IDLE_TTL_MS) },
  };

  await prisma.$transaction([
    prisma.pomodoroFocusSessionMember.updateMany({
      where: { isActive: true, session: idleSessions },
      data: { isActive: false, leftAt: new Date() },
    }),
    prisma.pomodoroFocusSession.updateMany({
      where: idleSessions,
      data: { isActive: false },
    }),
  ]);
