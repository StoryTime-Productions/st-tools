...
function makeOwnedSessionCollaboration(
  currentUserId: string
): TimerClientTestProps["initialCollaboration"] {
  return makeCollaboration({
    currentSession: {
      id: "session-1",
      title: "Deep Work",
...
describe("TimerClient", () => {
  ...
  it("renders owner collaboration controls and supports settings collapse", async () => {
    ...
    expect(screen.getByRole("button", { name: "Manage Teammate" })).toBeInTheDocument();
  ...
  it("shows an error toast when starting own session fails", async () => {
    actionMocks.startOwnFocusSessionAction.mockResolvedValueOnce({
    ...
    await waitFor(() => {
      expect(toastMocks.error).toHaveBeenCalledWith("Unable to start session");
    });
  });

  it("shows an error toast when joining a session fails", async () => {
    const props = makeProps();
    props.initialCollaboration = makeCollaboration({
      activeSessions: [
        {
    ...
    actionMocks.refreshPomodoroCollaborationAction.mockResolvedValue({ error: "offline" });
    actionMocks.joinFocusSessionAction.mockResolvedValueOnce({ error: "Unable to join session" });

    render(<TimerClient {...props} />);

    fireEvent.click(screen.getByRole("button", { name: "Join" }));
    await waitFor(() => {
      expect(toastMocks.error).toHaveBeenCalledWith("Unable to join session");
    });
  });

  it("shows an error toast when leaving a session fails", async () => {
    const props = makeProps();
    props.initialCollaboration = makeOwnedSessionCollaboration(props.currentUserId);

    actionMocks.leaveFocusSessionAction.mockResolvedValueOnce({
    ...
    await waitFor(() => {
      expect(toastMocks.error).toHaveBeenCalledWith("Unable to leave session");
    });
  });

  it("shows an error toast when adding a member to a session fails", async () => {
    const props = makeProps();
    props.initialCollaboration = makeOwnedSessionCollaboration(props.currentUserId);

    actionMocks.addUserToFocusSessionAction.mockResolvedValueOnce({
    ...
    await waitFor(() => {
      expect(toastMocks.error).toHaveBeenCalledWith("Unable to send request");
    });
  });

  it("shows an error toast when kicking a member fails", async () => {
    const props = makeProps();
    props.initialCollaboration = makeOwnedSessionCollaboration(props.currentUserId);
    ...
    actionMocks.kickFocusSessionMemberAction.mockResolvedValueOnce({
      error: "Unable to remove member",
    });

    render(<TimerClient {...props} />);

    await userEvent.click(screen.getByRole("button", { name: "Manage Teammate" }));
    await userEvent.click(await screen.findByRole("menuitem", { name: "Remove from session" }));

    await waitFor(() => {
      expect(toastMocks.error).toHaveBeenCalledWith("Unable to remove member");
    });
  });

  it("shows an error toast when responding to a collaboration request fails", async () => {
    const props = makeProps();
    props.initialCollaboration = makeOwnedSessionCollaboration(props.currentUserId);

    actionMocks.respondToFocusSessionRequestAction.mockResolvedValueOnce({
    ...
    await waitFor(() => {
      expect(toastMocks.error).toHaveBeenCalledWith("Unable to respond to request");
    });
  });

  it("shows an error toast when syncing shared timer state fails", async () => {
    const props = makeProps();
    props.initialCollaboration = makeOwnedSessionCollaboration(props.currentUserId);

    actionMocks.syncFocusSessionTimerStateAction.mockResolvedValueOnce({
      error: "Unable to sync timer state",
    });
